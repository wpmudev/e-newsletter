...
//            $this->load_template( 'page-unsubscribe.php' );
    ...
    function check_bounces() {
        ...
        $email_password = $this->_decrypt($this->settings['bounce_password']);
     ...
     **/
     function save_settings( $settings ) {
		...
		if ( isset($settings['bounce_password']) && !empty($settings['bounce_password'])) {
    ...
    function get_unsubscribe_member( $limit = '' ) {
        global $wpdb;
        $results = $wpdb->get_results( "SELECT * FROM {$this->tb_prefix}enewsletter_members WHERE unsubscribe_code = '' OR unsubscribe_code IS NULL" . $limit, "ARRAY_A" );
        return $results;
    }
